...
export default function About() {
      ...
      <section className='mx-auto max-w-6xl w-full min-h-screen flex flex-col md:flex-row items-center justify-center' id='about'>
        ...
        >
          <img
            ...
            alt='profile photo'
            className='cursor-pointer max-w-xs rounded-full ring-8 ring-white/70 shadow-xl shadow-slate-300/50 hover:shadow-teal-700/25 hover:shadow-2xl transition-all duration-300'
          />
        </motion.div>
 ...
 const AboutTextSection = ({SpecificClasses, isOpenEmail, setIsOpenEmail}) => {
  const aboutClasses = SpecificClasses+' flex-col items-center justify-center text-slate-800'
  return (
    <motion.div
        ...
        >
          <div className='cursor-default flex flex-col justify-center align-middle items-left max-w-2xs gap-1 mr-10'>
            <span className='w-fit text-xs bg-teal-100 text-teal-800 p-2 rounded-md py-1 hidden'>Open to hire</span>
            <h1 className='text-3xl font-bold bg-gradient-to-r from-teal-700 to-amber-600 text-transparent bg-clip-text'>
              José Antonio López
            </h1>
            <p className='text-lg bg-gradient-to-r font-light from-teal-800 to-slate-600 text-transparent bg-clip-text from-5% to-95%'>
